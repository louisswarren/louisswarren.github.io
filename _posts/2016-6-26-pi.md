...
3. Add an empty file called `ssh` on the boot partition

4. Start the Pi and ssh in, username `pi`, password `raspberry`.

5. Run `raspi-config`, set the hostname, reboot.

6. Get updates/upgrades.

7. Add `smsc95xx.turbo_mode=N` to `/boot/cmdline.txt` (don't know if still
   necessary).
